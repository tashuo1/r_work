...
当前阶段：**一期 Spring Boot + MyBatis 分层单体已替代旧微服务基座，后续开发以 `ontology-lite` 为准。**
数据库配置：运行配置默认从 `ontology-lite/runtime-config/ontology-platform.env` 读取；当前平台持久化库优先使用 MySQL/PolarDB，启动脚本会在外部库不可达时自动切换到本地 H2 兜底，保证一期产品页面可演示。
## 工作空间结构
